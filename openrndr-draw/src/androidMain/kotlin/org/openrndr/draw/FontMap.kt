...
fun loadFontImageMap(
    ...
    return if (isValidUrl(fileOrUrl)) {
        ...
        val file = File(AppContextHolder.context.filesDir, fileOrUrl)
        require(file.exists()) {
...
actual val defaultFontMap by lazy {
    ...
    val defaultFontFile = File(AppContextHolder.context.filesDir, defaultFontPath)
    if (defaultFontFile.isFile) {
        logger.info { "loading default font from ${defaultFontFile.absolutePath}" }
        loadFontImageMap(defaultFontPath, 16.0, contentScale = RenderTarget.active.contentScale)
    } else {
        logger.warn { "default font ${defaultFontFile.absolutePath} not found" }
        null
    }
}
