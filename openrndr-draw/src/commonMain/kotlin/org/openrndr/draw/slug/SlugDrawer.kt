...
vec2 SlugDilate(vec4 pos, vec2 tex, vec4 jac, vec4 m0, vec4 m1, vec4 m3, vec2 dim, out vec2 vpos) {
    ...
    fun draw(drawer: Drawer, slugMap: SlugMap) {
        drawer.isolated {
            this@SlugDrawer.shadeStyle.parameter("bands", slugMap.bands)
            this@SlugDrawer.shadeStyle.parameter("curves", slugMap.curves)

            drawer.shadeStyle = this@SlugDrawer.shadeStyle
