...
class RandomOrderer : Orderer {
    override fun order(node: PackNode, rectangle: IntRectangle): List<Int> {
        return (node.children.indices).shuffled()
    }
}
...
class DefaultSplitter : Splitter {
    override fun split(node: PackNode, rectangle: IntRectangle): List<PackNode> {
        ...
        if (dw > dh) {
            ...
            rrect = IntRectangle(
                node.area.x + rectangle.width + sanding,
                node.area.y,
                node.area.width - rectangle.width - sanding,
                node.area.height
            )
        } else {
            ...
            rrect = IntRectangle(
                node.area.x,
                node.area.y + rectangle.height + sanding,
                node.area.width,
                node.area.height - rectangle.height - sanding
            )
        }
        ...
        if (dw > dh) {
            ...
            if (xcon(node, rectangle)) {
                if (s * (node.area.x - enclosement.x) > s * enclosement.width / 2) {
                    lrect = IntRectangle(node.area.x, node.area.y, rectangle.width, node.area.height)
                    rrect = IntRectangle(
                        node.area.x + rectangle.width,
                        node.area.y,
                        node.area.width - rectangle.width,
                        node.area.height
                    )
                } else {
            ...
            if (ycon(node, rectangle)) {
                if (s * (node.area.y - enclosement.y) > s * enclosement.height / 2) {
                    ...
                    rrect = IntRectangle(
                        node.area.x,
                        node.area.y + rectangle.height,
                        node.area.width,
                        node.area.height - rectangle.height
                    )
                } else {
...
class GreedySplitter : Splitter {
    ...
    override fun split(node: PackNode, rectangle: IntRectangle): List<PackNode> {
        ...
        for (j in 0 until vc) {
            for (i in 0 until hc) {
                rects[j * hc + i] =
                    IntRectangle(xpos[i].toInt(), ypos[j].toInt(), widths[i].toInt(), heights[j].toInt())
            }
        }
