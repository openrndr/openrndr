...
internal class BezierQuadraticSampler2DT {
        ...
        if (d > colinearityEpsilon) {
            ...
            if (d * d <= distanceToleranceSquare * (dx * dx + dy * dy)) {
                ...
                if (angleTolerance < angleToleranceEpsilon) {
                    if (points.last().xy.squaredDistanceTo(x123.xy) > 0.0) {
                        points.add(x123)
                    }
                    return
                }
    ...
    fun sample(x1: Vector2, x2: Vector2, x3: Vector2): Pair< List<Vector2>, List<Double> > {
        distanceToleranceSquare = distanceTolerance * distanceTolerance
        points.clear()
        points.add(x1.vector3(z=0.0));
        sample(x1.vector3(z=0.0), x2.vector3(z=0.5), x3.vector3(z=1.0), 0)
        if (points.last().xy.squaredDistanceTo(x3) > 0.0) {
            points.add(x3.vector3(z=1.0));
        }
