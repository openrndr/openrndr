...
internal class BezierCubicSampler2DT {
    ...
    fun sample(x1: Vector2, x2: Vector2, x3: Vector2, x4: Vector2): Pair<List<Vector2>, List<Double>> {
        distanceToleranceSquare = distanceTolerance * distanceTolerance
        points.clear()
        points.add(x1.vector3(z = 0.0))
        sample(x1.vector3(z = 0.0), x2.vector3(z = 1.0 / 3.0), x3.vector3(z = 2.0 / 3.0), x4.vector3(z = 1.0), 0)
        if (points.last().xy.squaredDistanceTo(x4) > 0.0) {
            points.add(x4.vector3(z = 1.0));
        }
        return Pair(points.map { it.xy }, points.map { it.z })
    }
            ...
            0 -> {
                ...
                if (d2 > d3) {
                    if (d2 < distanceToleranceSquare) {
                        if (points.last().xy.squaredDistanceTo(x2.xy) > 0.0) {
                            points.add(x2)
                        }
                        return
                    }
                    ...
                    if (d3 < distanceToleranceSquare) {
                        if (points.last().xy.squaredDistanceTo(x3.xy) > 0.0) {
                            points.add(x3)
                        }
                        return
                    }
            ...
            1 ->
                ...
                if (d3 * d3 <= distanceToleranceSquare * (dx * dx + dy * dy)) {
                    if (angleTolerance < angleToleranceEpsilon) {
                        if (points.last().xy.squaredDistanceTo(x23.xy) > 0.0) {
                            points.add(x23)
                        }
                        return
                    }
