...
fun difference(from: ShapeContour, subtract: ShapeContour) : List<ShapeContour> {
    val result = from.toRing2().region().difference(subtract.toRing2().region())
    return result.rings.map { it.toShapeContour() }

}

fun union(from: ShapeContour, add: ShapeContour) : List<ShapeContour> {
    val result = from.toRing2().region().union(add.toRing2().region())
    return result.rings.map { it.toShapeContour() }
}

fun intersection(from: ShapeContour, with: ShapeContour) : List<ShapeContour> {
