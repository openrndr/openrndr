...
class CompoundBuilder {
    ...
    inner class OpBuilder {
        ...
        fun difference(f: OpBuilder.() -> Unit) {
            operands.add(this@CompoundBuilder.difference(f))
        }

        /** describe a org.openrndr.shape.union */
        fun union(f: OpBuilder.() -> Unit) {
            operands.add(this@CompoundBuilder.union(f))
        }

        /** describe an intersection */
        fun intersection(f: OpBuilder.() -> Unit) {
    ...
    /** describe a difference */
    fun difference(f: OpBuilder.() -> Unit) : List<Shape> {
        val ob = OpBuilder()
        ob.f()
    ...
    /** describe a org.openrndr.shape.union */
    fun union(f: OpBuilder.() -> Unit) : List<Shape> {
        val ob = OpBuilder()
        ob.f()
        return ob.union
    }
    ...
    /** describe an intersection */
    fun intersection(f: OpBuilder.() -> Unit) : List<Shape> {
        val ob = OpBuilder()
        ob.f()
